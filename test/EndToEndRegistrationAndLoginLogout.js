...
var randomNumber = Math.floor(Math.random()*10000);                          //Declared/Initialized variable to generate random number for unique email
...
test('Verify New Account Registration', async t =>{
    await t
        ...
        await t
        .typeText(RegistrationPage.registrationPageEmail,data.email+randomNumber+emailSuffix)      //Enter: Uniquely generated email
...
testDataSet.forEach(data =>{
    test('Verify Registered User Login and Logout', async t =>{
        await t
            ...
            .typeText(LoginPage.loginPageUserEmail,data.email+randomNumber+emailSuffix)     //Enter: Already registered user email
